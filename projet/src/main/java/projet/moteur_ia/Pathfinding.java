...
public class Pathfinding {
    ...
    Pathfinding(int numberNodes){
        ...
        this.pathDijkstraHeuristic=new int [numberNodes];
        ...
        for(int i=0;i<numberNodes;i++){
            ...
            this.pathDijkstraHeuristic[i]=-1;
        }
     ...
     * @return boolean
     */
    public boolean euclideanDistanceComparator(int noeud1,int noeud2,int noeud3){

        double x=(double)this.graph.listNodes.get(noeud1).x;
        double y=(double)this.graph.listNodes.get(noeud1).y;

        double xprime=(double)this.graph.listNodes.get(noeud2).x;
        double yprime=(double)this.graph.listNodes.get(noeud2).y;
        ...
        xprime=(double)this.graph.listNodes.get(noeud3).x;
        yprime=(double)this.graph.listNodes.get(noeud3).y;
    ...
    /** Method which implements dijkstra algorithm with heuristic, 
     ...
     * @return boolean (true if a path exists)
     */
    public boolean dijkstraWithHeuristic(int start,int arrival){
        ...
        while(indice<this.numberNodes) {
            ...
            indice+=1;}
        distance[currentNode]=0;
        this.pathDijkstraHeuristic[start]=start;
        boolean noPath=false;
        while(numberVisited<numberNodes && currentNode!=arrival && noPath==false){
                ...
                for(int j=0;j<sizeLinkedList;j++){
                    if(this.distance[graph.list.get(currentNode).get(j).indice]==-1 && currentNode==start){
                        this.pathDijkstraHeuristic[this.graph.list.get(currentNode).get(j).indice]=currentNode;
                        this.distance[graph.list.get(currentNode).get(j).indice]=0;
                        this.distance[graph.list.get(currentNode).get(j).indice]+=this.graph.list.get(currentNode).get(j).cost;
     ...
     * @param start 
     * @param arrival
     * @param algorithm (1 = dijkstra, 2=dijkstraWithHeuristic)
    ...
    public LinkedList<int[]> pathfinding(int start,int arrival,int algorithm){
        ...
        if(algorithm==1){
            ...
            if(dijkstraHeuristicComputed){pathExist=true;}
                ...
                if(this.dijkstraWithHeuristic(start, arrival)){
                    pathExist=true;
                }
            }
        }
        ...
        while(indice!=start){
            ...
            else if(algorithm==2) {indice=this.pathDijkstraHeuristic[indice];}
        }
        coord=new int[2];
        coord[0]=this.graph.listNodes.get(indice).x;
        coord[1]=this.graph.listNodes.get(indice).y;
     ...
     * @param start
     * @param arrival
     * @param method (0 for dijkstra 1 for dijkstraWithHeuristic)
     * @return LinkedList<int[]> (LinkedList of coordonate)
     */
